...
> Documento gerado a partir da leitura da wiki do projeto. Baseado **exclusivamente** na documentação `.md` deste repositório — quando algo não está documentado, é sinalizado como gap e listado na Seção 9.
>
...
flowchart TB
  ...
  subgraph V3["Backend v3 — Java + Spring Modulith"]
    MAgenda[Agenda]
    MAtv[Atividade]
...
- **12 repos no workspace FE** (em `D:\local\ftd\ftd-frontend\`): 1 monolítico + 1 host + 5 remotes domínio + 1 login standalone + 4 placeholders vazios no Azure DevOps.
...
- **Tecnologia de composição** (Module Federation? single-spa? import maps? iframe?). Wiki não cita nenhuma. A confirmar abrindo `package.json` + config (vite/webpack/next) do provável host `mfe-ftd-ionica_portal`.
- **Roteamento entre MFEs**.
- **Contrato host↔remote**: mount function, props, lifecycle, event bus, contexto compartilhado.
- **Mecanismo de descoberta de remotes**: URLs hardcoded, `remoteEntry.js`, manifesto, import maps.
- **Shared deps** entre host e remotes (React singleton, DS, auth, router).
- **Deploy**: hipótese forte de **deploy por MFE** (cada repo com seu pipeline) — confirmar olhando CI/CD em cada repo.
- **Onde mora o design system hoje** já que `mfe-ftd-ionica_componentes` está vazio. Ver Seção 9, bloco "Infraestrutura compartilhada".
...
4. **`mfe-ftd-ionica_home`** será dashboard separado do `portal` ou redesign futuro do landing/home atual?

### 🚨 Críticas pro frontend (perguntar no primeiro 1:1)

1. ~~**Existe arquitetura de microfrontends em runtime hoje, ou é um app React único monolítico?**~~ **Respondido em 2026-05-14:** existe MFE em runtime — 7 repos `mfe-*` populados (ver Seção 2). Hipótese de host: `portal`. Wiki segue desatualizada (`Módulos.md` continua 0 B — vale PR). Sub-perguntas 2 (tecnologia de composição) e 3 (build tool) **seguem abertas** — serão respondidas na Camada 1.
2. Se sim, **qual tecnologia de composição**? Module Federation? single-spa? iframes? Web Components? Build-time?
3. **Qual o build tool e o framework**? Vite? Webpack? CRA? Next.js? Não documentado.
4. **Qual a stack de estado** (Redux/Zustand/Jotai/Context puro)? Não documentado.
5. **Qual a estratégia de estilização** (Tailwind/CSS-in-JS/Modules/SCSS)? Não documentado — só "folhas de estilo" genéricas.
6. **Qual a stack de testes**? Unit (Jest/Vitest)? E2E (Cypress/Playwright)? A planilha de IDs sugere E2E mas a ferramenta não é nomeada.
7. **Qual o gerenciador de pacotes e se há monorepo** (Nx/Turborepo/pnpm workspaces/yarn workspaces)?
8. **Qual a URL/spec do BFF** e como o FE descobre/consome os endpoints? Há cliente gerado a partir do OpenAPI?
9. **Estratégia de auth**: JWT em `localStorage` está documentado, mas **onde está o login**, qual o refresh token strategy, e por que `localStorage` em vez de cookie httpOnly?
...
2. **Auth — caminhos divergentes.** `login` e `monolithic` autenticam no mesmo Azure AD B2C, mas por caminhos diferentes — `login` via OAuth2 redirect cru (sem lib), `monolithic` via libs `@azure/msal-*`. Por que duas implementações? Plano de unificar?
3. **Auth — B2C + JWT.** Auth real do projeto combina Azure AD B2C (confirmado em uso) + JWT em `localStorage` (documentado na Seção 5 da wiki)? São camadas complementares (B2C valida → emite JWT interno) ou redundantes? Como o token chega no `portal` vindo de `home.souionicahml.com`?
